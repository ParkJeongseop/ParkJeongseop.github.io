    ...
    function militaryService() {
        ...
        for (let i=0; i<promotions.length; i++) {
            if(new Date() <= promotions[i]) {
                ...
                enlist_span.innerHTML = Math.ceil((new Date() - enlistDate)/c);
                discharge_span.innerHTML = Math.floor((new Date() - dischargeDate)/c);
                promotion_span.innerHTML = Math.floor((new Date() - promotions[i])/c);
